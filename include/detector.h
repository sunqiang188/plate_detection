...
public:
    ...
    PlateDetector(const std::string& modelPath,
                 const bool& isGPU,
                 const cv::Size& inputSize);

    std::vector<PlateDetection> detect(cv::Mat &image);
    std::vector<PlateDetection> detect(std::vector<cv::Mat> images);
...
private:
    ...
    void preprocessing(cv::Mat &image, float*& blob, std::vector<int64_t>& inputTensorShape);
    std::vector<PlateDetection> postprocessing(std::vector<Ort::Value>& outputTensors);
